...
type Grid struct {
	...
	DatabaseName        string
	TableNames          string
	//	DatabaseNames       map[int8]string
	//	TableNames          map[int8]string
}
...
func gridInstance(c *Client) {
	...
	if _, err := os.Stat(dbtableFile); os.IsNotExist(err) {
		...
		*/
		...
		log.Printf("DbtableFile exists, open it.")
		...
		*/
		log.Printf("Here is grid.go, dbtablefile exists.")

		dbinfo, err := ioutil.ReadFile(dbtableFile)
		if err != nil {
			println(err)
		}
		var dbstr = string(dbinfo)
		dbstr1 := strings.Split(dbstr, ",")
		DatabaseName, TableName := dbstr1[0], dbstr1[1]
		log.Printf(DatabaseName, TableName)
		//grid.DatabaseNames[0] = DatabaseName
		//grid.TableNames[0] = TableName

		// Open database connection
		db, err := sql.Open("mysql", "root:123@/"+DatabaseName)
		if err != nil {
			fmt.Println("Error opening mysql database "+DatabaseName, err)
			log.Printf("Error opening mysql database "+DatabaseName, err)
		...
		minRowSize := len(values)
		minColumnSize := len(columns)
		columnCount := defaultColumnCount
		rowCount := defaultRowCount
		if defaultColumnCount < minColumnSize {
			columnCount = minColumnSize
		}
		if defaultRowCount < minRowSize {
			rowCount = minRowSize
		}

		log.Printf("rowcount = %d, columnCount = %d\n", rowCount, columnCount)
		sheetSizes := []SheetSize{SheetSize{RowCount: rowCount, ColumnCount: columnCount}, SheetSize{RowCount: rowCount, ColumnCount: columnCount}}

		completeName := DatabaseName + "."
		completeName = completeName + TableName
		sheetNames := make(map[string]int8)
		sheetNames[completeName] = 0
		sheetNames["Sheet2"] = 1

		sheetList := []string{completeName, "Sheet2"}
		//grid.DatabaseNames[sheetNames[completeName]] = DatabaseName
		//grid.TableNames[sheetNames[completeName]] = TableName
		...
		for rows.Next() {
			...
			if gridrow == 1 {
				for i, col := range values {
					if col == nil {
						...
						dv.DataString = value
						dv.SheetIndex = 0
						grid.Data[strconv.Itoa(0)+"!"+indexToLetters(i+1)+strconv.Itoa(gridrow)] = dv
					}
				}
				gridrow = 2
			}
			...
			for i, col := range values {
				...
				if col == nil {
					...
					dv.ValueType = DynamicValueTypeString
					dv.DataString = value
					log.Printf(dv.DataString)
					dv.SheetIndex = 0
					grid.Data[strconv.Itoa(0)+"!"+indexToLetters(gridcol)+strconv.Itoa(gridrow)] = dv
				}
